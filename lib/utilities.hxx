...
namespace kalmanAtlas
{
  ...
  affineTransformToMatrix(typename itk::AffineTransform<CoordRepType, NDimensions>::Pointer trans)
  {
    ...
    vnlMatrixType m(NDimensions+1, NDimensions+1, 0.0);

    typedef itk::AffineTransform<CoordRepType, NDimensions> AffineTransformType;
    typedef typename AffineTransformType::MatrixType ItkMatrixType;
    typedef itk::Vector<CoordRepType, NDimensions> ItkVectorType;

    ItkMatrixType affineMatrix = trans->GetMatrix();
    ItkVectorType offsetVector = trans->GetOffset();

    for (unsigned int r = 0; r < NDimensions; ++r)
      {
        m(r, NDimensions) = offsetVector[r]; // last col for tranlation

          for (unsigned int c = 0; c < NDimensions; ++c)
          {
            m(r, c) = affineMatrix(r, c);
          }
      }


    // being a homogeneous coord, the bottom-right corner is always 1.0
    m(NDimensions, NDimensions) = 1.0;
  ...
  matrixToAffineTransform(vnl_matrix<CoordRepType> m, const typename itk::AffineTransform<CoordRepType, NDimensions>::ParametersType& fixedParameter)
  {
    ...
    typedef itk::Vector<CoordRepType, NDimensions> ItkVectorType;
    ItkVectorType offsetVector(0.0);

    for (unsigned int r = 0; r < NDimensions; ++r)
      {
        offsetVector[r] = m(r, NDimensions);

        for (unsigned int c = 0; c < NDimensions; ++c)
          {
            affineMatrix(r, c) = m(r, c);
          }
      }


    // Order MATTERS!!!! If switch SetFixedParameters with SetOffset, will be wrong!!!
    trans->SetMatrix(affineMatrix);
    trans->SetFixedParameters(fixedParameter);
    trans->SetOffset(offsetVector);
  ...
  {
    vnl_vector<CoordRepType> v(16, 0.0);
    char id = 0;
    for (char i = 0; i < 4; ++i)
      {
        for (char j = 0; j < 4; ++j)
  ...
  {
    vnl_matrix<CoordRepType> m(4, 4, 0.0);
    char id = 0;
    for (char i = 0; i < 4; ++i)
      {
        for (char j = 0; j < 4; ++j)
  ...
  {
    vnl_matrix<CoordRepType> stateTransitionMatrix(16, 16, 0.0);
    for (int it = 0; it < 4; ++it)
      {
        for (int itt = 0; itt < 4; ++itt)
          {
            stateTransitionMatrix(4*it, 4*itt) = m(it, itt);
            stateTransitionMatrix(4*it + 1, 4*itt + 1) = m(it, itt);
            stateTransitionMatrix(4*it + 2, 4*itt + 2) = m(it, itt);
            stateTransitionMatrix(4*it + 3, 4*itt + 3) = m(it, itt);
          }
      }
  ...
  template< typename CoordRepType>
  vnl_matrix<CoordRepType>
  stateTransitionMatrixToHomogeneousMatrix(vnl_matrix<CoordRepType> m)
  {
    vnl_matrix<CoordRepType> homogeneousMatrix(4, 4, 0.0);

    for (int it = 0; it < 4; ++it)
      {
        for (int itt = 0; itt < 4; ++itt)
          {
            homogeneousMatrix(it, itt) = m(4*it, 4*itt);
          }
      }
  ...
  invertDiagonalMatrix(const vnl_matrix<CoordRepType>& m)
  {
    ...
    // check if diagonal
    for (unsigned int it = 0; it < nr; ++it)
      {
        for (unsigned int it1 = 0; it1 < nr; ++it1)
          {
            if (it == it1)
              {
                continue;
            ...
            if (fabs(m(it, it1)) > 1e-10)
              {
                std::cerr<<"Error: matrix is not diagonal\n";
                std::cerr<<"at ( "<<it<<", "<<it1<<" ), value is "<<m(it, it1)<<std::endl;
                abort();
              }
          }
      }
    ...
    vnl_matrix<CoordRepType> im(nr, nr, 0.0);

    for (unsigned int it = 0; it < nr; ++it)
      {
        if (fabs(m(it, it)) < 1e-10 )
          {
            std::cout<<"Warrning: diagonal element close to 0.\n";
          }

        im(it, it) = 1.0/m(it, it);
      }
  ...
  duplicateAffineTrans(typename itk::AffineTransform<CoordRepType, NDimensions>::Pointer trans)
  {
    ...
    /**
     * NOTE: the order of the following two settings does not
     * matter. However, if using
     * SetFixedParameters+SetMatrix+SetOffset, then the order
     * matters. See ../kalmanAtlas/testDuplicateTrans.cxx for detail
     */
    typename AffineTransformType::Pointer newTrans = AffineTransformType::New();
    newTrans->SetFixedParameters(trans->GetFixedParameters());
    newTrans->SetParameters( trans->GetParameters() );
